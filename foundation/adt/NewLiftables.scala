      ...
      val args = leaf.fields.map(f => {
        ...
        val value = {
          ...
          if (!f.tpe.baseClasses.contains(symbolOf[scala.collection.immutable.Seq[_]])) reify
            ...
            def extractDummy(any: Any): Option[String] = if (any.isInstanceOf[_root_.scala.reflect.core.Name]) Some(any.asInstanceOf[_root_.scala.reflect.core.Name].value) else None
